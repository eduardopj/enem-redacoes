...
app.use((req: Request, res: Response, next: NextFunction) => {
  (req as Request & { requestId: string }).requestId = String(req.headers['x-request-id'] || randomUUID());
  ...
  requestContext.run({ requestId: (req as Request & { requestId: string }).requestId }, next);
});
...
app.get('/health', async (req: Request, res: Response) => {
  let dbOk = true;
  try { await queryOne('SELECT 1'); } catch { dbOk = false; }
  const ok = dbOk;
...
const correctionLimiter = makeRateLimiter({
  ...
  legacyHeaders: false,
  store: rateLimitStore as any, // eslint-disable-line @typescript-eslint/no-explicit-any
...
(Sentry as { setupExpressErrorHandler?: (app: Application) => void }).setupExpressErrorHandler?.(app);

app.use((err: Error & { type?: string }, req: Request, res: Response, _next: NextFunction) => {
  captureException(err, { requestId: (req as Request & { requestId?: string }).requestId, path: req.originalUrl });
