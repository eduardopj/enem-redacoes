 ...
 * @param {string} essayId - Used as the storage key / filename
 * @param {Buffer} buffer  - WebP image bytes
 * @param {string} localPath - Absolute path where local storage saves the file
 * @returns {Promise<{ imagePath: string | null, imageUrl: string | null }>}
 *   imagePath: filesystem path (null for S3 mode)
 *   imageUrl:  public CDN/S3 URL (null for local mode — Express serves it)
 */
export async function storeImage(essayId, buffer, localPath) {
  if (env.s3Bucket) {
 ...
 */
export function getImagePublicUrl(essayId, backendUrl) {
  if (env.s3Bucket) {
...
function storeLocally(buffer, localPath) {
...
async function storeInS3(essayId, buffer) {
  const key = `${env.s3KeyPrefix}${essayId}.webp`;
  ...
  try {
    ...
    writeLog('error', 's3_upload_failed', { essayId, key, error: err.message });
    throw err;
  }
}
