#!/usr/bin/env node
/**
 * image-retention.ts — Limpeza periódica de imagens de redações.
 *
 * Duas operações:
...
function deleteFile(filePath: string, reason: string, meta: Record<string, unknown> = {}): void {
  try {
    rmSync(filePath);
    log('info', reason, { filePath, ...meta });
  } catch (err) {
    log('warn', 'delete_failed', { filePath, error: (err as Error).message });
  }
}
 ...
 */
function cleanOrphanedImages(db: DatabaseType): OrphanStats {
  let files: string[];
  try {
  ...
  const knownIds = new Set<string>(
    (db.prepare(`SELECT id FROM essays WHERE imagePath IS NOT NULL`).all() as { id: string }[]).map((r) => r.id)
  );
 ...
 */
function enforceQuotas(db: DatabaseType): QuotaStats {
  const teachers = db
    .prepare(
    ...
    .all(MAX_IMAGES_PER_TEACHER) as { teacherId: string; cnt: number }[];
  ...
  for (const { teacherId, cnt } of teachers) {
    ...
    const candidates = db
      ...
      .all(teacherId, excess) as { id: string; imagePath: string }[];
