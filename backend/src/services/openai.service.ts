...
function buildImageModeContent(themeTitle: string, dataUrl: string): ContentPart[] {
  return [
    {
      type: 'text',
...
function buildTextModeContent(themeTitle: string, essayText: string): string {
...
export async function correctEssayWithOpenAI({ themeTitle, imageBase64, mimeType, essayText }: CorrectEssayParams): Promise<ReturnType<typeof normalizeCorrection>> {
  if (!env.openAiApiKey) {
...
feedback: 2-3 frases de orientação principal para reescrita/próxima redação (direto e encorajador).
      ...
      },
      {
        ...
        content: isTextMode
          ...
          : (buildImageModeContent(themeTitle, dataUrl!) as unknown as string),
      },
    ],
  });
