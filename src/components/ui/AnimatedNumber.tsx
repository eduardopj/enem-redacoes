...
type Props = {
  ...
  duration?: number;
  /** Delay em ms antes de iniciar a animação (ex: 500 para aguardar transição de tela) */
  delay?: number;
...
export function AnimatedNumber({
  ...
  useEffect(() => {
    ...
    const run = () => {
      ...
      // ─── Modo suspense ──────────────────────────────────────────────────────
      // Fase 1 (52%): sprint 0 → 87% do valor
      // Fase 2 (48%): crawl 87% → 100% com desaceleração exponencial
      const inflectionValue = Math.round(value * 0.87);
      const phase1Duration = Math.round(duration * 0.52);
    ...
    const timer = delay > 0 ? setTimeout(run, delay) : (run(), undefined);
  ...
  }, [animVal, value, duration, delay, suspense]);
